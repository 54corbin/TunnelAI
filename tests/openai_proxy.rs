...
use tunnel_ai::client::start_openai_client_for_test;
...
use tunnel_ai::server::start_openai_server_for_test;
...
async fn openai_error_response_for_request(request: &str) -> String {
    let provider = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let provider_addr = provider.local_addr().unwrap();
    let provider_task = tokio::spawn(async move {
        timeout(Duration::from_millis(500), provider.accept())
...
fn chunked_body(response: &[u8]) -> Vec<u8> {
